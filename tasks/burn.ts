...
task('burn', 'Burn tokens in the issuing account')
  ...
  .setAction(async ({ amount }, hre) => {
    ...
    const account = ethAddressToXdc(
      await (await hre.ethers.getSigners())[0].getAddress(),
    );
    console.log(
      `Burning ${formatNumberString(amount)} tokens from issuing account ${account}`,
    );
    const amountWei = hre.ethers.utils.parseEther(amount);

    console.log(
      'total supply before burning: ',
      formatNumberString(await getTotalSupply(crescite, hre)),
    );
    console.log(
      'balance of account before burning: ',
